...
class LLMClient:
    ...
    def _lmstudio_chat(self, model, messages, temperature, on_token):
        ...
        try:
            ...
            raise LLMError(f"Erreur pendant l'appel à LM Studio ({model}) : {e}") from e
        except (json.JSONDecodeError, UnicodeDecodeError) as e:
            raise LLMError(
                f"Réponse invalide reçue de LM Studio ({model}) : flux interrompu ou JSON malformé. Détail : {e}"
            ) from e
